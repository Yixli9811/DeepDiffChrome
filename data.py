...
def loadData(filename,windows,gene_dict):
	...
	for i in range(0,nrows,windows):
		...
		ac1=torch.zeros(windows,1)
		ac2=torch.zeros(windows,1)
		for w in range(0,windows):
			...
			ac1[w][0]=int(data[i+w][6])
			ac2[w][0]=int(data[i+w][7])
...
class HMData(Dataset):
	...
	def __getitem__(self,i):
		final_data_c1=torch.cat((self.c1[i]['hm1'],self.c1[i]['hm2'],self.c1[i]['hm3'],self.c1[i]['hm4'],self.c1[i]['hm5'], self.c1[i]['ac1'],self.c1[i]['ac2']),1)
		final_data_c2=torch.cat((self.c2[i]['hm1'],self.c2[i]['hm2'],self.c2[i]['hm3'],self.c2[i]['hm4'],self.c2[i]['hm5'], self.c2[i]['ac1'],self.c2[i]['ac2']),1)
